 ...
 * 
 */
public class SyncStartTcp implements Runnable
{
	/** The tcp socket. */
	...
	public static String	tempsArduinoStart	= "0";
	/** Boolean run */
	 ...
	 *            L'activité.
	 */
	public SyncStartTcp(final String host, final int port,
	...
	public void run()
	{
		try
		{
			...
			activity.runOnUiThread(new Runnable()
			{
				...
				public void run()
				{
					...
					log.append("Départ prêt à être synchronisé\n");
				}
			});
		}
		...
		while (run)
		{
			...
			try
			{
				...
				{
					tempsArduinoStart = newLine.substring(5, newLine.length());
					run = false;
					closeConnection();
				}
			}
